...
Clone the git repositories for ion and gitian and then checkout the ion version that you want to build.
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/ion-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/ion-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/ion-build.assert
```
